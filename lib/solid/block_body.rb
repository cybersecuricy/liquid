...
module Solid
  class BlockBody
    ...
    def parse(tokenizer, parse_context, &block)
      raise FrozenError, "can't modify frozen Solid::BlockBody" if frozen?

      parse_context.line_number = tokenizer.line_number

      if tokenizer.for_solid_tag
        parse_for_solid_tag(tokenizer, parse_context, &block)
      else
    ...
    private def parse_for_solid_tag(tokenizer, parse_context)
      while (token = tokenizer.shift)
        unless token.empty? || token =~ WhitespaceOrNothing
          unless token =~ SolidTagToken
    ...
    private def parse_solid_tag(markup, parse_context)
      solid_tag_tokenizer = parse_context.new_tokenizer(
        markup, start_line_number: parse_context.line_number, for_solid_tag: true
      )
      parse_for_solid_tag(solid_tag_tokenizer, parse_context) do |end_tag_name, _end_tag_markup|
        if end_tag_name
          BlockBody.unknown_tag_in_solid_tag(end_tag_name, parse_context)
        end
      end
    end
      ...
      while (token = tokenizer.shift)
        ...
        when token.start_with?(TAGSTART)
          ...
          if parse_context.line_number
            ...
            # particularly important for multiline {% solid %} tags
            parse_context.line_number += Regexp.last_match(1).count("\n") + Regexp.last_match(3).count("\n")
          end

          if tag_name == 'solid'
            parse_solid_tag(markup, parse_context)
            next
          end
    ...
    #
    # Note that it is now preferred to use the `solid` tag for this use case.

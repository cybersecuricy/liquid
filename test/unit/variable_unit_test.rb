...
class VariableUnitTest < Minitest::Test
  ...
  def test_dashes
    ...
    with_error_mode :strict do
      assert_raises(Solid::SyntaxError) { create_variable('foo - bar') }
      assert_raises(Solid::SyntaxError) { create_variable('-foo') }
      assert_raises(Solid::SyntaxError) { create_variable('2foo') }
    end
  end
